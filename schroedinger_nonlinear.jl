...
## nonlinearities (Nemytskii kernels)
ϕ(z) = z/(1+abs(z)^2)
...
function euler(R,ts,FΔW,Fu0)
    ...
    for (i,t) in enumerate(ts[2:end])
        drift_term .= -1im * dt * applyFourierFunc(ϕ, FU[:,i]) # F(u) = -i ϕ(u)
...
@info "Nonlinear Schroedinger" β t_end dt_exact M N_samples
...
path = mkpath("data")
writedlm(joinpath(path, "NLSE_β_$(Int(10β))_T_$(t_end)_dt0_$(Int(log2(dt_exact)))_M_$(Int(log2(M)))_samples_$(N_samples).csv"), data)
