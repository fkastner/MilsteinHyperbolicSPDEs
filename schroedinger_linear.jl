...
## potential
# FV = zeros(M) # no potential
V = periodicbump.(xx) # smooth bump function centered at zero with compact support
FV = dft(V)
...
function euler(R, ts, FΔW, Fu0)
    ...
    for (i, t) in enumerate(ts[2:end])
        drift_term .= -1im * dt * conv_trunc!(tmp_conv, FV, FU[:, i]) # F(u) = -i V⋅u
...
@info "Linear Schroedinger with potential" β t_end dt_exact M N_samples
...
path = mkpath("data")
writedlm(joinpath(path, "SE_β_$(Int(10β))_T_$(t_end)_dt0_$(Int(log2(dt_exact)))_M_$(Int(log2(M)))_samples_$(N_samples)_V_bump.csv"), data)

## convergence plot
fig = Figure()
ax = Axis(fig[1, 1],
    title="SE: β = $(β), dt₀ = 2^($(log2(dt_exact))), M = 2^($(log2(M))), V: bump",
    xlabel="Stepsize",
